...
from .utils import get_data_iterator, TrainCollator, to_device
from .synthetic import SyntheticTrainDataset, SyntheticTNTestDataset
from .weather import WeatherTrainDataset, WeatherTestDataset
...
def load_data(config, device, split='trainval'):
    ...
    if config.data == 'synthetic':
        TrainDataset, TestDataset = SyntheticTrainDataset, SyntheticTNTestDataset
    elif config.data == 'weather':
        TrainDataset, TestDataset = WeatherTrainDataset, WeatherTestDataset
    else:
    ...
    if split == 'train' or split == 'trainval':
        train_data = TrainDataset(config.data_path, 'train', config.tasks, config.split_ratio, config.ts_train)
        train_collator = TrainCollator(config.ts_train, config.cs_range_train, config.gamma_train, config.tasks)
    ...
    if split == 'valid' or split == 'trainval':
        valid_data = TestDataset(config.data_path, 'valid', config.tasks, config.split_ratio,
    ...
    # load test loader
    if split == 'test':
        test_data = TestDataset(config.data_path, 'test', config.tasks, config.split_ratio,
