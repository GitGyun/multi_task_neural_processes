...
class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        if len(x.size()) == 6:
            B, N, O = x.size()[:3]
            x = x.view(B*N*O, *x.size()[3:])
            view = True
            
        x = self.conv(x)
        
        if view:
            x = x.view(B, N, O, *x.size()[1:])
...
class UpConv(nn.Module):
    ...
    def forward(self, x):
        B, M, T = x.size()[:3]
        x = x.reshape(B*M*T, *x.size()[3:])
        x = self.upconv(x)
        x = x.reshape(B, M, T, *x.size()[1:])
        
        return x

    
class AdaIN(nn.Module):
    def __init__(self, in_channels, out_channels, dim_hidden):
        super().__init__()
        self.channels = out_channels
        
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.instnorm = nn.InstanceNorm2d(out_channels)
        self.styler = nn.Linear(dim_hidden, 2*out_channels)
    ...
    def forward(self, x, h):
        ...
        if len(h.size()) == 3:
            scale, bias = self.styler(h).split(self.channels, 2) # B, T, K
            scale = scale.unsqueeze(1).unsqueeze(4).unsqueeze(4)
            bias = bias.unsqueeze(1).unsqueeze(4).unsqueeze(4)
        elif len(h.size()) == 4:
            scale, bias = self.styler(h).split(self.channels, 3) # B, M, T, K
            scale = scale.unsqueeze(4).unsqueeze(4)
            bias = bias.unsqueeze(4).unsqueeze(4)
        else:
